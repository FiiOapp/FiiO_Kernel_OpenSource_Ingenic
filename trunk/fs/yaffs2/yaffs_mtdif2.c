				      ...
				      const yaffs_ExtendedTags * tags)
{
	...
	if (data) {
#if defined(CONFIG_YAFFS_ECC_RS) || defined(CONFIG_YAFFS_ECC_HAMMING)
		writeOOBLen = sizeof(pt);
#else
		writeOOBLen = sizeof(pt) - sizeof(yaffs_ECCOther);
#endif

     		nandmtd2_pt2buf(dev, &pt, 0);         //modify
		ops.mode = MTD_OOB_AUTO;
		ops.ooblen = writeOOBLen;
...
#if defined(CONFIG_YAFFS_ECC_RS) || defined(CONFIG_YAFFS_ECC_HAMMING)
		readOOBLen = sizeof(pt);
#else
		readOOBLen = sizeof(pt) - sizeof(yaffs_ECCOther);
#endif

		ops.mode = MTD_OOB_AUTO;
		ops.ooblen = readOOBLen;
		ops.len = data ? dev->nDataBytesPerChunk : readOOBLen;
		ops.ooboffs = 0;
		ops.datbuf = data;
...
int nandmtd2_IsNANDBlockBad(struct yaffs_DeviceStruct *dev, int blockNo) //Add by xlsu
{
	struct mtd_info *mtd = (struct mtd_info *)(dev->genericDevice);

	T(YAFFS_TRACE_MTD,
	  (TSTR("nandmtd2_IsNANDBlockBad %d" TENDSTR), blockNo));

	return mtd->block_isbad(mtd,
