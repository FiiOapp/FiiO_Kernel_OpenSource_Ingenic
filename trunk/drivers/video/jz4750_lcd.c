...
#elif defined(CONFIG_JZ4750_SLCD_POWERTIP_TFT_PH128128T)
#include "st7715r.h"
#endif
...
//		 .cfg = LCD_CFG_LCDPIN_SLCD | LCD_CFG_RECOVER | /* Underrun recover*/ 
		 ...
		 LCD_CFG_NEWDES | /* 8words descriptor */
		 LCD_CFG_MODE_SLCD, /* TFT Smart LCD panel */
		 .slcd_cfg = SLCD_CFG_DWIDTH_8BIT_x3 | SLCD_CFG_CWIDTH_8BIT | SLCD_CFG_CS_ACTIVE_LOW | SLCD_CFG_RS_CMD_LOW | SLCD_CFG_CLK_ACTIVE_FALLING | SLCD_CFG_TYPE_PARALLEL,
		 .ctrl = LCD_CTRL_OFUM | LCD_CTRL_BST_16,	/* 16words burst, enable out FIFO underrun irq */
		 128, 128, 110, 0, 0, 0, 0, 0, 0,
	 },
	.osd = {
		 .osd_cfg = LCD_OSDC_OSDEN | /* Use OSD mode */
//		 LCD_OSDC_ALPHAEN | /* enable alpha */
...
//		 LCD_OSDC_F1EN | /* enable Foreground1 */
		 ...
		 .ipu_restart = 0x80001000, /* ipu restart */
		 .fg_change = FG_CHANGE_ALL, /* change all initially */
		 .fg0 = {32, 0, 0, 128, 128}, /* bpp, x, y, w, h */
		 .fg1 = {32, 0, 0, 128, 128}, /* bpp, x, y, w, h */
	 },
...
static void ctrl_disable(void)
{
	...
	} else if (jz4750_lcd_info->panel.cfg & LCD_CFG_LCDPIN_SLCD) {
...
static int jz4750fb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case FBIO_SET_TVE_MODE:
		...
		break;
#endif
	case FBIO_GET_BUFFER:
//		D("fbio get current buffer\n");
		if (copy_to_user(argp, &jz4750_lcd_info->current_buffer, sizeof(int))) {
			printk(KERN_ERR "user get current buffer failed!\n");
			return -EFAULT;
		}
		break;
	default:
...
{
	int next_frm = 0;
	volatile unsigned int uncached_addr = 0;
	...
	if (var->xoffset - cfb->fb.var.xoffset) {
		printk(KERN_ERR "No support for X panning for now!\n");
		return -EINVAL;
	}

	next_frm = var->yoffset / var->yres;
	lcd_info->current_buffer = next_frm;
	if (next_frm >= lcd_info->frm_nr) {
		printk(KERN_ERR "yoffset is invalid!\n");
		return -EINVAL;
	}
//	D("var.yoffset: %d, next_frm: %d", var->yoffset, next_frm);

	uncached_addr = CKSEG1ADDR(&dma0_desc0->databuf);
	REG32(uncached_addr) = (unsigned int)virt_to_phys((void *)lcd_frame0 + lcd_info->frm_size * next_frm);
			...
			struct fb_info *info)
{
	...
	var->yres_virtual           = var->height * CONFIG_FB_JZ4750_FB_NUMS;
	var->xoffset                = 0;
	var->yoffset                = 0;
	var->pixclock               = 0;
	var->left_margin            = 0;
...
static int jz4750fb_map_smem(struct lcd_cfb_info *cfb)
{
	...
	jz4750_lcd_info->osd.fg0.w = w;
	jz4750_lcd_info->osd.fg0.h = h;
#endif
	jz4750_lcd_info->current_buffer = 0;
	jz4750_lcd_info->frm_nr = CONFIG_FB_JZ4750_FB_NUMS;
	jz4750_lcd_info->frm_size = ((w * bpp + 7) >> 3) * h;
	printk("========> {%s, %d} w = %d, bpp = %d, h = %d <========\n", __func__, __LINE__, w, bpp, h);
	needroom1 = needroom = jz4750_lcd_info->frm_size * jz4750_lcd_info->frm_nr;
	...
	jz4750_lcd_info->osd.fg1.w = w;
	jz4750_lcd_info->osd.fg1.h = h;
#endif
	needroom += ((w * bpp + 7) >> 3) * h;
#endif // two layer
	D("FG0 Size = %d, FG1 Size = %d, FB Nums = %d\n", needroom1, needroom - needroom1, CONFIG_FB_JZ4750_FB_NUMS);

	for (page_shift = 0; page_shift < 12; page_shift++)
		if ((PAGE_SIZE << page_shift) >= needroom)
			break;
	if (PAGE_SIZE << page_shift < needroom) {
		printk(KERN_WARNING "Framebuffer size(%d) is too large!\n", needroom);
		jz4750_lcd_info->frm_nr = (PAGE_SIZE << page_shift - needroom1) / jz4750_lcd_info->frm_size;
	}
	D("Framebuffer size = %d, Framebuffer num = %d\n", jz4750_lcd_info->frm_size, jz4750_lcd_info->frm_nr);
	...
	pi = (unsigned int *)lcd_cmdbuf;
	pc = (unsigned char *)lcd_cmdbuf;
	data = WR_GRAM_CMD;

	switch (jz4750_lcd_info->panel.slcd_cfg & SLCD_CFG_CWIDTH_MASK) {
...
static void jz4750fb_unmap_smem(struct lcd_cfb_info *cfb)
{
	...
	needroom = jz4750_lcd_info->frm_size * jz4750_lcd_info->frm_nr;
#if defined(CONFIG_FB_JZ4750_LCD_USE_2LAYER_FRAMEBUFFER)
...
static void jz4750fb_descriptor_init( struct jz4750lcd_info * lcd_info )
{
	...
	dma0_desc_cmd0->cmd 		= LCD_CMD_CMD | 1; /* command */
	dma0_desc_cmd0->offsize 	= 0;
	dma0_desc_cmd0->page_width 	= 0;
	dma0_desc_cmd0->cmd_num 	= 1;
		...
		if ( lcd_info->osd.fg_change & FG0_CHANGE_SIZE ) { /* change FG0 size */
			if ( lcd_info->panel.cfg & LCD_CFG_TVEN ) { /* output to TV */
				...
				dma0_desc0->page_width = dma0_desc1->page_width 
					= fg0_line_size/4;
				dma0_desc1->databuf = virt_to_phys((void *)(lcd_frame0 + fg0_line_size * lcd_info->frm_nr));
				REG_LCD_DA0 = virt_to_phys(dma0_desc0); //tft
			}
			else {
...
static void jz4750fb_change_clock( struct jz4750lcd_info * lcd_info )
{
	...
	if ((lcd_info->panel.cfg & LCD_CFG_MODE_MASK) == LCD_CFG_MODE_SLCD) {
		int time = 0;
		switch (lcd_info->panel.slcd_cfg & SLCD_CFG_DWIDTH_MASK) {
		case SLCD_CFG_DWIDTH_8BIT_x3:
			time = 3;
			break;
		case SLCD_CFG_DWIDTH_8BIT_x2:
		case SLCD_CFG_DWIDTH_9BIT_x2:
			time = 2;
			break;
		default:
			time = 1;
		...
		/* smart lcd控制器内部两个cycle发一次数据，所以必须要再乘以2!!! */
		pclk = 2 * time * val * lcd_info->panel.w * lcd_info->panel.h;/* Pixclk */
	} else if( (lcd_info->panel.cfg & LCD_CFG_MODE_MASK) != LCD_CFG_MODE_SERIAL_TFT) {
	...
	if ( lcd_info->panel.cfg & LCD_CFG_TVEN ) { 		/* LCDC output to TVE */
		...
		val--;
		D("ratio: val = %d\n", val);
		printk("===> __cpm_get_pllout2() = %d, pclk = %d, ratio: val = %d\n", __cpm_get_pllout2(), pclk, val);
		if ( val > 0x7ff ) {
...
//	ptr = (int *)lcd_frame1;
	...
	dma_cache_wback_inv(lcd_frame0, w * h * 4);
}
#if 0
