...
static int sg_to_desc(struct scatterlist *sgentry, JZ_MSC_DMA_DESC *first_desc,
	...
	/* Early JZ SOC DMA Memaddress NEED align to word */
	if (max_burst_dma_addr % 4) {
		printk("Address is not multiple of 4., address = %d\n", max_burst_dma_addr);
		return -EFAULT;
	}
...
static irqreturn_t jz_mmc_dma_callback(int irq, void *devid)
{
	...
	int chan = host->dma.channel;
#ifdef JZ_DMA_HAS_DCCSR_CT
	u32 dmac_dccsr_value = REG_DMAC_DCCSR(chan);
#endif

#ifdef JZ_DMA_HAS_DCCSR_CT
	if (dmac_dccsr_value & (DMAC_DCCSR_AR | DMAC_DCCSR_TT | DMAC_DCCSR_HLT))
		disable_dma(chan);
#else
	disable_dma(chan);
#endif
...
#ifdef JZ_DMA_HAS_DCCSR_CT
	if (dmac_dccsr_value & DMAC_DCCSR_CT) {
		__dmac_channel_clear_count_terminated(chan);
	}
