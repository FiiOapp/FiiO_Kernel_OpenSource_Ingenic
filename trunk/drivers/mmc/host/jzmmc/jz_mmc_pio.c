/*
 * linux/drivers/mmc/host/jz_mmc/msc/jz_mmc_pio.c
 */
...
static int jz_mmc_pio_read(struct jz_mmc_host *host, unsigned char *buf, size_t len) {
	...
	for (i = 0; i < len_v; i++) {
		...
		while ((timeout > 1) &&
		       ((REG_MSC_STAT(host->pdev_id) & MSC_STAT_DATA_FIFO_EMPTY) &&
		       !(REG_MSC_STAT(host->pdev_id) & MSC_STAT_TIME_OUT_READ) && !host->eject)) {
			msleep(1);
			timeout --;
		}
		if (timeout == 1) {
			printk("wait fifo not empty timedout!\n");
			printk("REG_MSC_IREG:\t%08x\n", REG_MSC_IREG(host->pdev_id));
			printk("REG_MSC_STAT:\t%08x\n", REG_MSC_STAT(host->pdev_id));
			return -ETIMEDOUT;
		}
...
void jz_mmc_start_pio(struct jz_mmc_host *host) {
	...
	host->transfer_end = 0;
	host->k = kthread_run(jz_mmc_data_transfer, (void *)host, "msc pio transfer");
}
