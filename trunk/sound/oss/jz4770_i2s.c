...
//	printk("%%%% format = %d\n", format);
	...
	case AFMT_S24_LE:
	case AFMT_U24_LE:
		sound_data_width = 32;       //actual width == 8bit 0 + 24bit valid sample data
		break;
	default:
...
/*
 * Convert stereo data to mono data, data width: 32 bits/channel
 *
 * buff:        buffer address
 * data_len:    data length in kernel space, the length of stereo data
 *              calculated by "node->end - node->start"
...
int convert_32bits_stereo2mono(void *buff, int data_len)          
{
        ...
        int data_len_64aligned = data_len & ~0x3f;
        int data_cnt_ushort = data_len_64aligned / 4;
        int mono_cur, stereo_cur;
...
static inline int endpoint_set_filter(audio_pipe *endpoint, int format, int channels)
{
	...
	case AFMT_S24_LE:
		if (endpoint == &in_endpoint) {
			if (channels == 1) {
				endpoint->filter = convert_32bits_stereo2mono;
				DPRINT("$$$$ set pin_endpoint->filter = convert_32bits_stereo2mono\n");
			} else {
				endpoint->filter = NULL;
				DPRINT("$$$$ set pin_endpoint->filter = NULL\n");
			}
...
static int jz_audio_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SNDCTL_DSP_GETFMTS:
		...
		rc = put_user(AFMT_U8 | AFMT_S8 | AFMT_S16_LE | AFMT_U16_LE | AFMT_S24_LE | AFMT_U24_LE, (int *)arg);
		break;
...
//		printk("\nSNDCTL_DSP_SETFMT ... set to %d\n", val);
	...
	case SOUND_PCM_READ_BITS:
		if (mode & CODEC_RMODE) {
			...
			else if(codec->record_format & (AFMT_S16_LE | AFMT_U16_LE)){
					format = 16;
			}
			else if(codec->record_format & (AFMT_S24_LE  | AFMT_U24_LE )){
					format = 24;
			}else{
					format = 24;
			}
		...
		if (mode & CODEC_WMODE) {
			if(codec->replay_format & (AFMT_S8 | AFMT_U8)){
					format = 8;
			}
			else if(codec->replay_format & (AFMT_S16_LE | AFMT_S16_BE | AFMT_U16_LE | AFMT_U16_BE)){
					format = 16;
			}
			else if(codec->replay_format & (AFMT_S24_LE  | AFMT_U24_LE )){
					format = 24;
			}else{
					format = 24;
			}
