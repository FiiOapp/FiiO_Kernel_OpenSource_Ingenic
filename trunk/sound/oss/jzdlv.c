...
*/
	...
	if(val == 0)
		...
		dlv_write_reg_bit(1, 0, 5);//DAC_MUTE->0
 ...
 */
static void dlv_set_record(unsigned long arg)
{
	ENTER();
	...
	if(arg == USE_MIC){
		...
		dlv_write_reg_bit(1, 0, 7);//MICBIAS ON 
	...
	}else if(arg == USE_LINEIN){
		dlv_write_reg(0x09, 0xff);
		dlv_write_reg(0x08, 0x2f);
		dlv_write_reg_bit(23, 0, 7);//AGC1.AGC_EN->0
		schedule_timeout(2);
		dlv_write_reg_bit(1, 0, 2);//CR1.BYPASS->0
		dlv_write_reg(0x16, 0xc2);//linein
		
		dlv_write_reg_bit(5, 0, 5);//PMR1.SB_MIX->0
		schedule_timeout(10);
		dlv_write_reg_bit(5, 1, 3);//PMR1.SB_LIN->1
		dlv_write_reg_bit(5, 1, 0);//PMR1.SB_IND->1
		dlv_write_reg_bit(1, 1, 7);//MICBIAS OFF
